...
export const Toggle = () => {
    ...
    return(
        ...
        <div className="bg-slate-950  flex flex-col h-screen justify-center items-center text-white" >
            ...
            <div className="flex items-center  justify-between rounded-full w-60 p-8" 
                 style={{backgroundColor:ToggleColor}}>
                <div 
                className={` border-4 border-slate-800  rounded-full p-5 duration-500 ${CheckISon} bg-[${ToggleColor}]`} 
                onClick={handleBtn}> {CheckISon}</div>
            </div>
        </div>
        </>
    );
